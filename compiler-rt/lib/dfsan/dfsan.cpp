...
// Current preceding branch instruction.
static thread_local int __dfsan_control_bi_id = -1;
...
__dfsan_control_scope_label (int unified) {
  if(__dfsan_control_bi_id >= __dfsan_control_array_size) {
    Report("FATAL: ControlFlowSanitizer: Scope label function call to a non-set id.");
    return 0;
  }
  if(unified)
    return __dfsan_control_unified_labels[__dfsan_control_bi_id];
  return __dfsan_control_split_labels[__dfsan_control_bi_id];
}
extern "C" SANITIZER_INTERFACE_ATTRIBUTE dfsan_label
